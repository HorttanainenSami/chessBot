...
const useMove = () => {
  ...
  const kingLegalMoves = ({fromBitIndex, occupiedBits, enemyOccupied}:{ fromBitIndex:number, occupiedBits: Long, enemyOccupied: Long }) => {
    //filter attacked squares
    const pseudoMoves = checkingRays.not().and(kingPseudoMoves({fromBitIndex}));
  ...
  const calculatePinned = (state:Long[],color: Color) => {
    ...
    const occupiedBits = state.reduce( (acc, curr) => acc.or(curr), new Long(0x0, 0x0, true));
    
    const teammateOccupiedBits = color === 'w'
      ? occupiedBits.xor(blackOccupiedBits) 
      : blackOccupiedBits;
    const pinned = pinnedPieces(color,teammateOccupiedBits, occupiedBits, state);
    setPinned(pinned);
  };
  ...
  const knightPseudoMoves = ({ fromBitIndex }: { fromBitIndex: number }) => {
    ...
    return checked 
      ? legalMoves.and(checkingRays)
      :legalMoves;
  };
  function isCheck(state:Long[], color:Color){
    const blackOccupiedBits = state.reduce((acc, curr, i) =>i % 2 === 0? acc:acc.or(curr), Long.UONE);
    const occupiedBits = state.reduce(
      (acc, curr) => acc.or(curr),
      Long.UONE
    );
    const teammateOccupiedBits = color==='b'?blackOccupiedBits: occupiedBits.xor(blackOccupiedBits);
    const enemyOccupied = color==='b'? occupiedBits.xor(blackOccupiedBits):blackOccupiedBits;
    const kingPosition = color ==='b'? state[11] as Long: state[10] as Long;
    const fromBitIndex = kingPosition.countTrailingZeros();
    if(squareIsAttacked({occupiedBits, fromBitIndex, teammateOccupiedBits, friendlyColor:color})){
      setChecked(true);
      const { rays, numOfChecks } = kingIsAttackedFrom({occupiedBits, fromBitIndex, teammateOccupiedBits, color});
      setCheckingRays(rays);
      if(numOfChecks >1) {
        console.log('doublecheck');
        setDoubleChecked(true);
        const filteredkMoves = kingLegalMoves({fromBitIndex, occupiedBits, enemyOccupied:occupiedBits.xor(teammateOccupiedBits)}).and(rays.not());
  ...
  const bishopLegalMoves = ({
    ...
    if(!piecePosition.and(pinned).isZero()){
      ...
      return getBishopPinnerDirection(fromBitIndex, kingPosition.countTrailingZeros(), piecePosition, occupiedBits);
    }
  ...
  const rookLegalMoves = ({
    ...
    if(!piecePosition.and(pinned).isZero()){
      ...
      const colorHelper = color ==='w'? 1:0;
      const kingPosition = gameState[11-colorHelper] as Long;
      const rookMask = moveMasks.getBishopMoves(kingPosition.countTrailingZeros());
      //if bishop is attacking, rook cannot move
      if(!piecePosition.and(rookMask).isZero()){
        return Long.UZERO;
      }
      //check in what direction pinner is
      const legalMoves = getRookPinnerDirection(fromBitIndex, kingPosition.countTrailingZeros(), piecePosition, occupiedBits);
      return checked 
        ? legalMoves.and(checkingRays)
        :legalMoves;
    }
  ...
  const rookLegalAttacks = ({
    ...
    //positive
    const NAttacks = removeBlockedMovesForward(fromBitIndex, NMoveMask, occupiedBits);
    const WAttacks = removeBlockedMovesForward(fromBitIndex, WMoveMask, occupiedBits);
    const EAttacks = removeBlockedMovesBackwards(fromBitIndex, EMoveMask, occupiedBits);
    const SAttacks = removeBlockedMovesBackwards(fromBitIndex, SMoveMask, occupiedBits);
    //negative
  ...
  const pawnPseudoAttacks = ({ fromBitIndex, color }: Omit<IpawnPseudoMoves, 'occupiedBits'>) => {
    let moveMask = Long.UZERO;
    if (color === 'b') {
      //eat left
  ...
  const pawnLegalMoves = ({
    ...
    if(!Long.UONE.shiftLeft(fromBitIndex).and(pinned).isZero()){
      const colorHelper = color ==='w'? 1:0;
      const kingPosition = gameState[11-colorHelper] as Long;
      return getPawnPinnerDirection(fromBitIndex, kingPosition.countTrailingZeros(), Long.UONE.shiftLeft(fromBitIndex),occupiedSquares, color, enemyOccupied);
    }
    const pseudoMoves = pawnPseudoMoves({ fromBitIndex, color });
    const pseudoMovesBlockerRemoved = color==='w' 
      ? removeBlockedMovesForward(fromBitIndex, pseudoMoves, occupiedSquares, true).and(pseudoMoves)
      : removeBlockedMovesBackwards(fromBitIndex, pseudoMoves, occupiedSquares, true).and(pseudoMoves);
  ...
  const kingIsAttackedFrom = ({
    ...
    const rookAttacks = rookLegalMoves({ color, occupiedBits, teammateOccupiedBits, fromBitIndex }).and(rooksQueens);
  ...
  const squareIsAttacked = ({
    ...
    if (!pawnPseudoAttacks({ fromBitIndex, color:friendlyColor }).and(ePawns).isZero()) {
      return true;
    }
    ...
    if (
      !rookLegalMoves({ color:friendlyColor, occupiedBits, teammateOccupiedBits, fromBitIndex })
        .and(eRooksQueens)
        .isZero()
    ) {
      return true;
  ...
  function bitScanForward(from: number, moveMask:Long, occupied:Long) {
    const moveMaskWithoutFromSquare = moveMask.sub(Long.UONE.shiftLeft(from));
  ...
  function removeBlockedMovesForward(from: number, moveMask: Long, occupied: Long, removeBlockers: boolean=false) {
    ...
    return removeBlockers ? maskBetween.xor(Long.UONE.shl(blockingPieceIndex)): maskBetween;
  }
  function removeBlockedMovesBackwards( from: number, moveMask: Long, occupied:Long, removeBlockers:boolean=false) {
  ...
  function xrayRookAttacks (occupied:Long, blockers: Long, square:number) {
    const occupiedWOSelf = occupied.sub(Long.UONE.shiftLeft(square));
    const attack = rookLegalAttacks({fromBitIndex:square, occupiedBits:occupied}).and(occupiedWOSelf);
    const onlyBlockers = blockers.and(attack);
    const attacksBehindBlockers = rookLegalAttacks({fromBitIndex:square, occupiedBits:occupied.xor(onlyBlockers)}).and(occupiedWOSelf);
  ...
  function xrayBishopAttacks (occupied:Long, blockers: Long, square:number) {
    ...
    const attack = bishopAttacks({fromBitIndex:square, occupiedBits:occupied}).and(occupiedWOSelf);
    const onlyBlockers = blockers.and(attack);
    const attacksBehindBlockers = bishopAttacks({fromBitIndex:square, occupiedBits:occupied.xor(onlyBlockers)}).and(occupiedWOSelf);
    return attack.xor(attacksBehindBlockers);
  }
  function pinnedPieces (color:Color, teammateOccupiedBits: Long, occupied: Long, state: Long[] ) {
    const colorHelper = color ==='w'?1:0;
  ...
  function getBishopPinnerDirection( square: number, kingSq: number, bishopPosition: Long, occupied:Long){
    if(square>kingSq){
      //NW
      if(!moveMasks.getNW(kingSq).and(bishopPosition).isZero()){
        return removeBlockedMovesForward(square, moveMasks.getNW(square), occupied);
      }
      //NE
      if(!moveMasks.getNE(kingSq).and(bishopPosition).isZero()){
        return removeBlockedMovesForward(square, moveMasks.getNE(square), occupied);
      }
    }
    //SW
  ...
  function getRookPinnerDirection( square: number, kingSq: number, rookPosition: Long, occupied:Long){
    if(square>kingSq){
      //N
      if(!moveMasks.getN(kingSq).and(rookPosition).isZero()){
        return removeBlockedMovesForward(square, moveMasks.getN(square), occupied);
      }
      //W
      if(!moveMasks.getW(kingSq).and(rookPosition).isZero()){
        return removeBlockedMovesForward(square, moveMasks.getW(square), occupied);
      }
    }
    //E
  ...
  function getPawnPinnerDirection( square: number, kingSq: number, pawnPosition: Long, occupied:Long, color: Color, enemyOccupied:Long){
    //N can move up if not blocked
    if(!moveMasks.getN(kingSq).and(pawnPosition).isZero() || !moveMasks.getS(kingSq).and(pawnPosition).isZero()){
      const mask =  pawnPseudoMoves({fromBitIndex: square, color});
      if( color ==='w') return removeBlockedMovesForward(square, mask, occupied, true);

      return removeBlockedMovesBackwards(square, mask, occupied, true);
    }
    //NW
    if(color==='w'&&!moveMasks.getNW(kingSq).and(pawnPosition).isZero()){
      return pawnLegalAttacks({ fromBitIndex: square, enemyOccupied, color }).and(moveMasks.getNW(kingSq));
    }
    //NE
    if(color==='w'&&!moveMasks.getNE(kingSq).and(pawnPosition).isZero()){
      return pawnLegalAttacks({ fromBitIndex: square, enemyOccupied, color }).and(moveMasks.getNE(kingSq));
    }
    //SW
    if(color==='b'&&!moveMasks.getSW(kingSq).and(pawnPosition).isZero()){
      return pawnLegalAttacks({ fromBitIndex: square, enemyOccupied, color }).and(moveMasks.getSW(kingSq));
    }
    //SE
    if(color==='b'&&!moveMasks.getSE(kingSq).and(pawnPosition).isZero()){
      return pawnLegalAttacks({ fromBitIndex: square, enemyOccupied, color }).and(moveMasks.getSE(kingSq));
    }
    return Long.UZERO;
  }
  return {
