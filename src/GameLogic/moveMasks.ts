...
const getNWMask = (fromBitIndex: number) => {
  ...
  const NWhelper = fileNumber-rankNumber >=0 ? fileNumber*8 :rankNumber*8;
  return antiDiagonalMask.shiftLeft(NWhelper).shiftRightUnsigned(NWhelper).shiftLeft(fromBitIndex);
};
...
const getNEMask = (fromBitIndex: number) => {
  ...
  return diagonalMask.shiftLeft(NEhelper).shiftRightUnsigned(NEhelper+7).shiftLeft(fromBitIndex);
};
...
const getSWMask = (fromBitIndex: number) => {
  ...
  return diagonalMask.shiftRightUnsigned(SWhelper).shiftLeft(SWhelper+7).shiftRightUnsigned(63-fromBitIndex);
...
const getSEMask = (fromBitIndex: number) => {
  ...
  return antiDiagonalMask.shiftRightUnsigned(SEhelper).shiftLeft(SEhelper).shiftRightUnsigned(63-fromBitIndex);
};
...
export const bishopLegalMoves = ({ teammateOccupiedBits, fromBitIndex, occupiedBits, possibleMoves, bishopPosition}: IbishopLegalMoves) => {
  ...
  const blockingPiecesNW = checkForADiagonalBlockingPieces({possibleMoves:possibleMoves.and(NWMask).xor(bishopPosition), occupiedSquares: occupiedBits});
  const blockingPiecesNE = checkForDiagonalBlockingPieces({possibleMoves:possibleMoves.and(NEMask).xor(bishopPosition), occupiedSquares: occupiedBits});
  const blockingPiecesSW = checkForDiagonalBlockingPieces({possibleMoves:possibleMoves.and(SWMask).xor(bishopPosition), occupiedSquares: occupiedBits});
  const blockingPiecesSE = checkForADiagonalBlockingPieces({possibleMoves:possibleMoves.and(SEMask).xor(bishopPosition), occupiedSquares: occupiedBits});

  //get blocking piece that is closest to moving piece
  const pseudoAttackNW = blockingPiece(blockingPiecesNW, NWMask);
  const pseudoAttackNE = blockingPiece(blockingPiecesNE, NEMask);
  const pseudoAttackSW = blockingPiece(blockingPiecesSW, SWMask, true);
  const pseudoAttackSE = blockingPiece(blockingPiecesSE, SEMask, true);

  const pseudoAttacks = pseudoAttackNW.or(pseudoAttackNE).or(pseudoAttackSW).or(pseudoAttackSE);
...
export const getNMask = (fromBitIndex: number) => HFileSet.shiftLeft(fromBitIndex).shiftLeft(8);
export const getSMask = (fromBitIndex: number) => {
  ...
  return AFileSet.shiftRightUnsigned((7-fileNumber)+(7-rankNumber)*8).shiftRightUnsigned(8);
};
...
export const getWMask = (fromBitIndex: number) => {
  const fileNumber = getFileNumber(fromBitIndex);
  const rankNumber = getRankNumber(fromBitIndex);
  return Rank1Set.shiftRightUnsigned(1+fileNumber).shiftLeft(rankNumber*8+fileNumber+1);
};
...
export const rookLegalMoves = ({rookMoveMask, fromBitIndex, occupiedSquares, teammateOccupiedBits }: IRookLegalMoves) => {
  ...
  //positive
  const northAttacks = blockingPiece(pseudoAttacks, NMoveMask);
  const westAttacks = blockingPiece(pseudoAttacks, westMoveMask);
  //negative
  const southAttacks = blockingPiece(pseudoAttacks, SMoveMask, true);
